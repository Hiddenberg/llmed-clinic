...
import { SessionMetrics, SessionMetricsListObject } from "../types/zoomVideoSDKTypes"
...
export async function getZoomSessionDetails (sessionId: string, type: "past" | "live" = "past") {
   try {
      const response = await axios.get<SessionMetrics>(`${ZOOM_VIDEO_SDK_ENDPOINTS.SESSIONS}/${doubleEncodeZoomSessionId(sessionId)}`, {
         headers: {
            Authorization: `Bearer ${ZOOM_VIDEO_SDK_TOKEN}`
         },
         params: {
...
export async function prepareZoomSessionLiveStream (zoomSessionId: string, streamUrl: string, streamKey: string) {
   try {
      ...
      const updateRequest = await axios.patch(`${ZOOM_VIDEO_SDK_ENDPOINTS.SESSIONS}/${doubleEncodeZoomSessionId(zoomSessionId)}/livestream`, {
         stream_url: streamUrl,
         stream_key: streamKey,
         page_url: ZOOM_BROADCAST_URL,
         resolution: '720p'
      },{
         headers: {
            Authorization: `Bearer ${ZOOM_VIDEO_SDK_TOKEN}`
      ...
      console.error(`[Zoom Video SDK Service] Error updating session live stream for zoom session ${zoomSessionId}`, updateRequest.data)
      return {
         success: false,
...
export async function getZoomSessionLiveStreamDetails (zoomSessionId: string) {
   try {
      const liveDetailsResponse = await axios.get(`${ZOOM_VIDEO_SDK_ENDPOINTS.SESSIONS}/${doubleEncodeZoomSessionId(zoomSessionId)}/livestream`, {
         headers: {
            Authorization: `Bearer ${ZOOM_VIDEO_SDK_TOKEN}`
         }
      })
...
export async function startZoomSessionLiveStream (zoomSessionId: string, sessionDisplayName: string) {
   try {
      ...
      const startRequest = await axios.patch(`${ZOOM_VIDEO_SDK_ENDPOINTS.SESSIONS}/${doubleEncodeZoomSessionId(zoomSessionId)}/livestream/status`, {
         action: "start",
         settings: {
...
export async function stopZoomSessionLiveStream (zoomSessionId: string) {
   try {
      const stopRequest = await axios.patch(`${ZOOM_VIDEO_SDK_ENDPOINTS.SESSIONS}/${doubleEncodeZoomSessionId(zoomSessionId)}/livestream/status`, {
         action: "stop"
      }, {
         headers: {
            Authorization: `Bearer ${ZOOM_VIDEO_SDK_TOKEN}`,
